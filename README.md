...
레바논 지역 공습/상황 정보를 지도 위에 등록·조회하는 웹 서비스입니다.
...
이 프로그램을 돌리려면 서버에 세 가지가 설치되어 있어야 합니다.
...
```
$ sudo apt update
$ sudo apt install -y git postgresql nodejs npm
```
...
원하는 위치(예: 홈 디렉터리)로 이동한 뒤 코드를 내려받습니다.

```
$ cd ~
$ git clone https://github.com/newjeans365/lebanon-map.git
$ cd lebanon-map
```
...
PostgreSQL이 처음 설치되면 `postgres`라는 사용자 계정이 자동으로 만들어집니다. 이 계정의 비밀번호를 설정합니다. (원하는 비밀번호로 `내비밀번호` 부분을 바꿔주세요)

```
$ sudo -u postgres psql -c "ALTER USER postgres PASSWORD '내비밀번호';"
```

### 4-2. 데이터 저장 공간(테이블) 만들기

이 프로젝트에는 필요한 테이블 구조가 `sql/schema.sql` 파일에 미리 정의되어 있습니다. 아래 명령어로 한 번에 만듭니다.

```
$ PGPASSWORD=내비밀번호 psql -U postgres -h localhost -d postgres -f sql/schema.sql
```

`CREATE TABLE`이라는 메시지가 여러 줄 나오면 성공입니다.

### 4-3. (선택) 강/휴전선/점령지 마커 초기 데이터 넣기

지도에 표시되는 리타니 강, Yellow/Blue Line, 51번 국도, 점령지 마커의 기본값을 한 번에 넣어주는 스크립트입니다. 처음 설치할 때 한 번만 실행하면 됩니다. (이미 데이터가 있으면 자동으로 건너뜁니다)
...
| 항목 | 설명 |
|---|---|
| `PORT` | 이 프로그램이 사용할 포트 번호. 특별한 이유가 없다면 `3001` 그대로 두면 됩니다. |
| `DB_USER` | PostgreSQL 접속 계정. 보통 `postgres` |
| `DB_HOST` | DB 서버 주소. 같은 서버 안에 있다면 `localhost` |
| `DB_NAME` | DB 이름. 보통 `postgres` |
...
터미널에 `Lebanon map server running on port 3001` 이라고 뜨면 성공입니다. `Ctrl + C`를 누르면 서버가 종료됩니다.

같은 서버 안에서 아래처럼 확인해볼 수 있습니다.

```
$ curl http://localhost:3001/api/locations
```

데이터(JSON 형식 텍스트)가 출력되면 정상입니다.
...
$ pm2 start server.js --name lebanon-map
```
...
```
$ pm2 status
$ pm2 logs lebanon-map
$ pm2 restart lebanon-map
```
...
서버의 공인 IP 주소로 접속합니다. (예: 서버 IP가 `1.2.3.4`라면)

- 지도 (상황 등록): `http://1.2.3.4:3001/`
- 데이터 관리 페이지 (조회/수정/삭제): `http://1.2.3.4:3001/admin.html`

방화벽 때문에 접속이 안 될 수 있습니다. `3001` 포트를 열어줘야 합니다. (`ufw` 사용 시)
...
매번 `:3001`을 붙이지 않고 `http://1.2.3.4/` 처럼 접속하고 싶다면, `nginx`로 80번 포트를 3001번으로 연결해주면 됩니다.

```
$ sudo apt install -y nginx
```

`/etc/nginx/sites-available/lebanon-map` 파일을 새로 만들고 아래 내용을 넣습니다.
...
```
$ sudo ln -s /etc/nginx/sites-available/lebanon-map /etc/nginx/sites-enabled/
$ sudo nginx -t
...
- **`node server.js` 했는데 DB 관련 에러가 뜬다** → `.env`의 `DB_PASSWORD` 등 값이 4-1에서 설정한 값과 일치하는지 확인하세요.
...
├── scripts/seed-map-features.js  # 강/휴전선/마커 초기 데이터 입력
