...
const db = (c) => getDb(c.env.YEMEN_DATABASE_URL);
...
routes.get('/', async (c) => {
  try {
    const result = await db(c).query(`
      SELECT id, name, dms_string, lat, lon, category, detail_info, damage_info, attacker,
...
routes.post('/', async (c) => {
  ...
  try {
    const result = await db(c).query(
      `INSERT INTO locations (name, dms_string, lat, lon, category, detail_info, damage_info, attacker, occurred_at)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING *`,
      [name, dms_string, lat, lon, category, detail_info, damage_info, attacker, occurred_at]
    );
  ...
  try {
    const result = await db(c).query(
      `UPDATE locations SET name=$1, dms_string=$2, lat=$3, lon=$4, category=$5, attacker=$6,
       detail_info=$7, damage_info=$8, occurred_at=$9
       WHERE id=$10 RETURNING *`,
      [name, dms_string, lat, lon, category, attacker, detail_info, damage_info, occurred_at, id]
    );
