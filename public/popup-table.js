/**
 * @file popup-table.js
 * @description "우크라이나 지도 만들기" 팝업의 복붙용 표 생성 및 지도 캡처 기능
...
function attackerPriority(attacker) {
    ...
    if (a.includes('러시아')) return 0;
    if (a.includes('우크라이나')) return 1;
...
async function loadPopupTable() {
    ...
    filtered.forEach(item => {
        ...
        tr.innerHTML = `
            <td class="col-order ${item.attacker === '러시아군' ? 'russia' : 'ukraine'}">${order}</td>
