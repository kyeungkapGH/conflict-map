/**
 * @file popup-table.js
 * @description "예멘 지도 만들기" 팝업의 복붙용 표 생성 및 지도 캡처 기능
...
// 페이지 로드 시 기본 선택값(사우디 주도 연합군) 동기화
...
function attackerPriority(attacker) {
    ...
    if (a.includes('사우디')) return 0;
    if (a.includes('후티')) return 1;
...
async function loadPopupTable() {
    ...
    // 2) 공격 주체별 순서 초기화
    const counter = { '사우디 주도 연합군': 0, '후티 반군': 0 };
    ...
    filtered.forEach(item => {
        ...
        tr.innerHTML = `
            <td class="col-order ${item.attacker === '사우디 주도 연합군' ? 'saudi' : 'houthi'}">${order}</td>
