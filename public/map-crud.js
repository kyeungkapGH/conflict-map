 ...
 * @requires constants.js (UNIFIL_COORDS)
 */
...
function renderList(data) {
    ...
    list.innerHTML = sorted.map((item) => {
        const dist    = getDistance(parseFloat(item.lat), parseFloat(item.lon));
        ...
        return `
            <li class="list-item">
                ...
                <div class="item-details">
                    <div style="font-weight:bold; color:${isClose ? 'red' : 'black'}">
...
/**
 * 두 좌표 간의 거리를 Haversine 공식으로 계산한다. (단위: km)
 *
 * @param {number} lat - 대상 위도
 * @param {number} lon - 대상 경도
 * @returns {number} 동명부대 기준 거리 (km)
 */
function getDistance(lat, lon) {
    const R       = 6371;
    const baseLat = UNIFIL_COORDS[1];
    const baseLon = UNIFIL_COORDS[0];
